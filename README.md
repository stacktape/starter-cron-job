...
> [!TIP]
> To deploy this project using **GUI-based flow**, navigate to [console](https://console.stacktape.com/create-new-project/git-project-using-console?name=my-stacktape-app&repositoryType=public&repositoryUrl=https://github.com/stacktape/starter-cron-job)

- Scheduled [Lambda function](https://docs.stacktape.com/compute-resources/lambda-functions/) that runs on a cron
  schedule.
- Perfect for recurring background tasks like sending emails, cleaning up data, generating reports, or syncing services.
...
```bash
stacktape init --starterId cron-job
```
...
[dev command](https://docs.stacktape.com/cli/commands/dev/). For example, to develop and debug lambda function `scheduledTask`, you can use

```bash
stacktape dev --region <<your-region>> --stage <<stage>> --resourceName scheduledTask
```
